...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $sel = mysqli_query($con,"SELECT a.MedicineID, a.MedicineName,b.* FROM pharmaceuticals a, medication b WHERE b.AdmissionID = '$id' AND b.MedicineID = a.MedicineID AND b.DosingID != 0");
    $data = array();
    while ($row = mysqli_fetch_array($sel)) {
        $data[] = array(
            "ID"=>$row['ID'],
...
if(isset($_GET['medid']) && isset($_GET['id'])) {
    ...
    $admissionid = $_GET['id'];
    $sel = mysqli_query($con,"SELECT a.MedicineID,a.MedicineName,a.Unit,b.* FROM pharmaceuticals a,medication b WHERE b.AdmissionID = '$admissionid' AND b.ID =  '$medicineid' AND a.MedicineID = b.MedicineID");
    $data = array();
    while ($row = mysqli_fetch_array($sel)) {
        $data[] = array(
            "MedicineID"=>$row['MedicineID'],
