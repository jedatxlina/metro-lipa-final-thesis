...
if(isset($_GET['medicationid']) && isset($_GET['medicalid'])) {
    ...
    $sel = mysqli_query($conn,"SELECT medical_details.MedicationID, medication.* FROM medical_details JOIN medication WHERE medical_details.MedicalID = '$medicalid' AND medical_details.MedicationID = '$medicationid' AND medical_details.MedicationID = medication.MedicationID AND  medication.Quantity = '0' AND medication.DosingID = '0'");
    $data = array();
    while ($row = mysqli_fetch_array($sel)) {
        $data[] = array(
            "MedicineName"=>$row['MedicineName'],
